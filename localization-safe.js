/* TMS 60 safe localization completion layer.
   Completes dynamic EN/DE/KO UI strings without adding a MutationObserver. */
'use strict';
(()=>{
  if(window.top===window||window.__TMS_SAFE_I18N__)return;
  window.__TMS_SAFE_I18N__=true;
  ...
  const D={
    ...
    ko:{
      ...
      'Light uses white surfaces. Dark uses a deep background tinted by the selected accent.':'라이트 모드는 밝은 화면을, 다크 모드는 선택한 강조 색상이 반영된 어두운 화면을 사용합니다.','Interface scale':'화면 크기','The app language changes menus and instructions. Your Bible version is a separate setting.':'앱 언어는 메뉴와 안내 문구를 변경합니다. 성경 번역본은 별도로 설정됩니다.',
  ...
  function localizeReference(s,l){
    ...
    for(const [en,names] of Object.entries(BOOKS).sort((a,b)=>b[0].length-a[0].length)){
      ...
      const re=new RegExp('(^|\\b)'+esc+'(?=\\s+\\d+:\\d)','g');
      out=out.replace(re,(_,p)=>p+names[l==='de'?0:1]);
    }
    return out;
  }
  ...
  function translateRoot(root,l){
    ...
    while((n=walker.nextNode())){
      ...
      const x=tr(n.nodeValue,l);if(x!==n.nodeValue)n.nodeValue=x;
    }
    root.querySelectorAll?.('[placeholder],[aria-label],[title]').forEach(el=>{
      for(const attr of ['placeholder','aria-label','title'])if(el.hasAttribute(attr)){
        const value=el.getAttribute(attr),x=tr(value,l);if(x!==value)el.setAttribute(attr,x);
      }
    });
  }
  function translateVisible(){
    const l=currentLang();document.documentElement.lang=l;if(l==='en')return;
    const roots=[document.querySelector('.sidebar'),document.querySelector('.topbar'),document.querySelector('.mobile-nav'),document.querySelector('.view.active'),document.getElementById('modal-root'),document.getElementById('toasts')];
    const seen=new Set();for(const root of roots){if(root&&!seen.has(root)){seen.add(root);translateRoot(root,l)}}
  }

  let raf=0;
  function schedule(){if(raf)return;raf=requestAnimationFrame(()=>{raf=0;translateVisible()})}
  function wrap(name){
    const original=window[name];if(typeof original!=='function'||original.__tmsI18nWrapped)return;
    const wrapped=function(...args){const value=original.apply(this,args);schedule();return value};
    wrapped.__tmsI18nWrapped=true;window[name]=wrapped;
  }
  ['renderHome','renderStudy','renderLibrary','renderProgress','renderSettings','renderAll','switchView','modal','toast'].forEach(wrap);
  document.addEventListener('click',()=>setTimeout(schedule,0));
  document.addEventListener('change',()=>setTimeout(schedule,0));
  document.addEventListener('keydown',()=>setTimeout(schedule,0));
