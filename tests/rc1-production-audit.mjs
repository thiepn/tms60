...
async function getAppFrame(page, timeout = 45000) {
  ...
  while (Date.now() < deadline) {
    ...
    if (frame) {
      try {
        await frame.waitForSelector('#desktop-nav', { timeout: 1500 });
        return frame;
      } catch (_) {}
    }
...
async function auditDesktopCore(browser) {
  ...
  if (langBox && bibleBox) {
    ...
    fail('Language/Bible settings layout measurable', 'Missing bounding box');
  }

  await frame.locator('#ui-language-select').click();
  await page.keyboard.press('Escape');
  await frame.locator('#shell-version-select').click();
  await page.keyboard.press('Escape');
  ...
  for (let i = 0; i < 8; i++) {
    for (const view of ['progress', 'library', 'study', 'settings', 'home']) await navigate(frame, view);
  }
  ...
  const exportButton = frame.locator('[data-action="export-progress"]');
  if (await exportButton.count()) {
    ...
    fail('Progress export control exists');
  }
  ...
  const regs = await page.evaluate(async () => {
    ...
    return { supported: true, count: r.length };
  });
...
const FORBIDDEN_UI = [
  ...
  'Reliance on God’s Resources', "Reliance on God's Resources", 'Being Christ’s Disciple', "Being Christ's Disciple",
  'Growth in Christlikeness', 'Read slowly, notice the structure', 'I read the verse aloud slowly.',
  'I said the book, chapter & verse before and after the verse.', 'I looked away and recalled at least the opening phrase.',
  'Due work is never hidden merely because it exceeds this target.', 'Removes canonical-order cues.',
  'Interface scale:', 'Private and offline', 'Saved locally', 'All packs', 'All statuses'
];
...
async function auditLocalizationAndSwitching(browser) {
  const app = await openApp(browser, { lang: 'en', version: 'esv' });
  let { context, page, frame } = app;
  await navigate(frame, 'settings');

  for (const lang of ['de', 'ko']) {
    const waitNav = page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 20000 });
  ...
  const waitNav = page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 20000 });
  await frame.locator('#ui-language-select').selectOption('en');
  await waitNav;
  frame = await getAppFrame(page);
  await navigate(frame, 'settings');
...
async function auditBibleVersions(browser) {
  const versions = [
    ['esv', 'ESV'], ['niv', 'NIV'], ['nlt', 'NLT'], ['hfa', 'HFA'],
    ['schlachter1951', 'SCH1951'], ['klb1985', 'KLB 1985'], ['krv1961', '개역한글']
  ];
...
async function auditOffline(browser) {
  ...
  try {
    await page.reload({ waitUntil: 'domcontentloaded', timeout: 30000 });
    const frame = await getAppFrame(page, 30000);
    detail = await frame.locator('.brand-title').innerText();
  } catch (error) {
    offlineOk = false;
...
async function auditMobile(browser) {
  ...
  const bibleBox = await frame.locator('[data-shell-version-settings]').boundingBox();
  if (langBox && bibleBox) {
    check(langBox.width >= 150 && bibleBox.width >= 150, 'Mobile language/Bible controls remain usable widths', `${Math.round(langBox.width)}px/${Math.round(bibleBox.width)}px`);
  }
