...
try{
  {
    ...
    const count=await inputs.count();
    test(count>=2,'Cloze exposes multiple blanks',String(count));
    ...
    await first.press('Tab');
    test(await second.evaluate(el=>document.activeElement===el),'Desktop Tab advances to next blank');
    await second.fill('');
    ...
    test(await frame.locator('.rating-row.qol-rating-ready').evaluate(el=>document.activeElement===el),'Final Enter checks and safely focuses rating group');
...

  {
    ...
    const inputs=frame.locator('.cloze-input:not(:disabled)');
    const count=await inputs.count();
    test(count>=2,'Mobile cloze exposes multiple blanks',String(count));
    ...
    test(await inputs.nth(1).evaluate(el=>document.activeElement===el),'Mobile Space advances to next blank');
    ...
    test(await inputs.nth(2).evaluate(el=>document.activeElement===el),'Mobile keyboard Next/Enter advances intermediate blank');
