...
async function waitForPreferences(frame){
...
async function auditPreferences(frame,label=''){
  await waitForPreferences(frame);
  const suffix=label?` ${label}`:'';
  check(await frame.locator('#ui-language-select').count()===1,`One language selector${suffix}`);
  check(await frame.locator('#shell-version-select').count()===1,`One Bible selector${suffix}`);
  check(await frame.locator('[data-shell-version-settings] #ui-language-select').count()===1,`Language selector stays with Bible selector${suffix}`);
  const options=await frame.locator('#shell-version-select option').count();
  check(options===7,`All seven Bible versions remain available${suffix}`,String(options));
}
...
async function desktopAudit(browser){
  ...
  await nav(frame,'settings');await auditPreferences(frame,'on first Settings open');
  ...
  await nav(frame,'settings');await auditPreferences(frame,'after 30-view navigation stress');const exp=frame.locator('[data-action="export-json"]');if(await exp.count()){const dl=page.waitForEvent('download',{timeout:10000}).catch(()=>null);await exp.click();check(Boolean(await dl),'JSON export downloads')}else check(false,'JSON export control exists');
...
async function mobileAudit(browser){for(const width of [390,768,1024]){const a=await open(browser,{viewport:{width,height:844}}),f=a.frame;if(width<=760){check(await f.locator('.mobile-nav').isVisible(),`Mobile nav visible ${width}px`);await f.locator('.mobile-nav [data-view="settings"]').click()}else await nav(f,'settings');await f.waitForFunction(()=>document.documentElement.dataset.view==='settings');await f.waitForTimeout(400);await auditPreferences(f,`${width}px`);const overflow=await f.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth);check(overflow<=2,`No horizontal overflow ${width}px`,`${overflow}px`);const card=await f.locator('[data-shell-version-settings]').boundingBox(),l=await f.locator('#ui-language-select').boundingBox(),b=await f.locator('#shell-version-select').boundingBox();check(Boolean(card&&l&&b),`Combined preference controls visible ${width}px`);if(card&&l&&b){check(l.width>=140&&b.width>=140,`Preference controls usable ${width}px`,`${Math.round(l.width)}/${Math.round(b.width)}`);check(l.x>=card.x-2&&l.x+l.width<=card.x+card.width+2&&b.x>=card.x-2&&b.x+b.width<=card.x+card.width+2,`Preference controls remain inside card ${width}px`)}check(!a.errors.length,`No runtime errors ${width}px`,a.errors.join(' | '));await a.context.close()}}
