...
const result=await frame.evaluate(()=>{
  ...
  const singleWordTypo=compareText('Galatians','Galations');
  ...
  return{
    ...
    finalizer:window.__TMS60_TYPO_SCORING_FINALIZER__,
    installs:window.__TMS60_TYPO_SCORING_INSTALLS__,
    compareHook:compareText?.name,
    referenceHook:checkReference?.name,
    clozeHook:checkCloze?.name,
    ...
    singleWordTypo:{score:singleWordTypo.score,minorTypoCount:singleWordTypo.minorTypoCount,majorErrorCount:singleWordTypo.majorErrorCount},
...
pass(result.finalizer==='1.0.0','Late scoring finalizer loads',String(result.finalizer));
pass(result.installs>=2,'Scoring hooks are reasserted after late runtime initialization',String(result.installs));
pass(result.compareHook==='typoAwareCompareTextFinal','Character-aware compare hook remains active after load',String(result.compareHook));
pass(result.referenceHook==='typoAwareCheckReference','Typo-aware reference hook remains active after load',String(result.referenceHook));
pass(result.clozeHook==='typoAwareCheckClozeFinal','Typo-aware cloze hook remains active after load',String(result.clozeHook));
...
pass(result.verseTypo.score>=90&&result.verseTypo.score<100&&result.verseTypo.minorTypoCount===1&&result.verseTypo.majorErrorCount===0,'Single-letter verse typo receives character-level partial credit',JSON.stringify(result.verseTypo));
pass(result.singleWordTypo.score>=90&&result.singleWordTypo.score<100&&result.singleWordTypo.minorTypoCount===1,'Even a one-word answer with one letter mistyped reaches Good',JSON.stringify(result.singleWordTypo));
